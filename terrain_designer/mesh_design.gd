...
func _ready() -> void:
	resolution = terrain_design_node.resolution
...
func generate_arrays(rect2: Rect2) -> Dictionary:
	...
	for i in range(resolution):
		for j in range(resolution):
			...
			world_pos.y = terrain_noise_node.get_altitude(world_pos)
			vertices.append(world_pos)
			normals.append(terrain_noise_node.get_normal(world_pos, step))
			var uv = Vector2(world_pos.x, world_pos.z) * (1/100) # e.g., 100 meters per UV tile
			uvs.append(uv)
